...
subtitle: "Visão geral de modelos lineares para dados longitudinais (continuação) - modelagem"
fontsize: 10pt
author: |
...
## Parâmetros substantivos e de incômodo para dados longitudinais {.allowframebreaks}
...
- A contabilização da correlação entre medidas repetidas completa a especificação de qualquer modelo de regressão para dados longitudinais.
    - Aumenta a eficiência ou a precisão com a qual os parâmetros de regressão podem ser estimados.
- Quando um modelo apropriado para a covariância é adotado
    - erros padrão corretos são obtidos;
    - inferências válidas sobre os parâmetros de regressão podem ser feitas.

##  Modelando a covariância

- Além disso, quando há \structure{dados ausentes}, a modelagem correta da covariância é frequentemente um requisito para obter estimativas válidas para os parâmetros de regressão.
...
- Esta abordagem permite qualquer padrão arbitrário de covariância entre as medidas repetidas.
...
## Estruturas de covariância de efeitos aleatórios {.allowframebreaks}
