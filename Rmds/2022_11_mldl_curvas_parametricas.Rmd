...
- Na __aula anterior:__ \structure{perfis de respostas}.
    + Não impõe nenhuma estrutura na tendência no tempo da resposta média. 
    + Abordagem interessante quando: \structure{(1)} todos os indivíduos foram medidos nas mesmas ocasiões; \structure{(2)} o número de ocasiões é relativamente pequeno.

\framebreak

- Conforme o número de ocasiões aumenta e as medidas repetidas são irregulares no tempo, analisar perfis de resposta se torna menos atraente.
+ Ainda, o teste da hipótese nula de __ausência de interação__ $grupo \times tempo$ é um teste global; 
    + A rejeição desta hipótese não fornece nenhum indicativo de um padrão de mudança da média ao longo do tempo.
- A resposta média ao longo tempo pode, em geral, ser descrita por \structure{curvas paramétricas} (__lineares__ ou __quadráticas__) ou \structure{semiparamétricas} (__splines lineares__) relativamente simples.

\framebreak

- É improvável que o padrão de mudança na resposta média ao longo de um estudo longitudinal seja tão complicado que sua descrição exija tantos parâmetros quanto as ocasiões de medição.
- A análise dos perfis de resposta usa um modelo __saturado__ para a resposta média ao longo do tempo e, portanto, produz um __ajuste perfeito ao perfil__ de resposta média observado.
...
- \structure{Substancialmente}, em muitos estudos longitudinais, o verdadeiro __processo de resposta média subjacente__ provavelmente __mudará ao longo do tempo__ em um __padrão__ relativamente __suave__, __monotonicamente crescente ou decrescente__, pelo menos durante a duração do estudo.
    + __Exemplo:__ crescimento da função pulmonar em criança e adolescentes.
...
- \structure{De uma perspectiva estatística}, o ajuste de modelos parcimoniosos para a resposta média resultará em testes estatísticos de efeitos de covariáveis (por exemplo, interações $tratamento \times tempo$) que têm __maior poder__ do que em uma análise de perfis de resposta.
...
## Tendências lineares no tempo {.allowframebreaks}

- A __curva mais simples possível__ para descrever mudanças na resposta média ao longo do tempo __é uma linha reta__.
+ Neste modelo, a inclinação do tempo tem interpretação direta em termos de uma __mudança constante__ na resposta média para uma mudança unitária no tempo:
    + Se $\E(Y_{ij}) = \beta_1 + \beta_2 t_{ij}$, então $\frac{d}{dt_{ij}}\E(Y_{ij}) = \beta_2$.
...
- $Grupo_i = 1$ se o $i$-ésimo indivíduo foi designado ao novo tratamento, e $Grupo_i = 0$ caso contrário;
- $Tempo_{ij}$ denota o tempo de medição para a $j$-ésima ocasião no $i$-ésimo indivíduo.

\framebreak

- No modelo linear descrito no slide anterior, a média dos indivíduos designados ao grupo controle é
...
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.height='50%', out.width='60%', paged.print=FALSE, purl=FALSE}
knitr::include_graphics(here::here('images', 'fig6_1.png'))
```

- Se o termo de interação é diferente de zero, então a mudança (a inclinação da reta) ao longo do tempo é específica de cada grupo de tratamento.

\framebreak

- Aqui $\color{darkviolet}\beta_1$ é o \textcolor{darkviolet}{intercepto no grupo de controle} (o grupo de "referência"), enquanto $\color{darkgreen}(\beta_1 + \beta_2)$ é o \textcolor{darkgreen}{intercepto no grupo de tratamento}.
...
-  Finalmente, a inclinação, ou \textcolor{darkviolet}{taxa constante de mudança} na resposta média por unidade de mudança no tempo, é $\color{darkviolet}{\beta_2}$ no \textcolor{darkviolet}{grupo controle}, enquanto a \textcolor{darkgreen}{inclinação} correspondente no \textcolor{darkgreen}{grupo de tratamento} é $\color{darkgreen}{(\beta_2 + \beta_4)}$.
- Normalmente, em um estudo longitudinal a questão de interesse primário diz respeito a uma comparação das mudanças na resposta média ao longo do tempo; __isso pode ser traduzido em uma comparação das inclinações__.
- Assim, se $\color{red}{\beta_4 = 0}$, então os dois grupos não diferem em termos de mudanças na resposta média ao longo do tempo.

## Tendências quadráticas no tempo {.allowframebreaks}
...
- Em vez disso, a taxa de mudança na resposta média depende do tempo:

se $\E(Y_{ij}) = \beta_1 + \beta_2 t_{ij} + \beta_3 t^2_{ij}$, então $\frac{d}{dt_{ij}}\E(Y_{ij}) = \beta_2 + 2\beta_3t_{ij}$.
...
- Os coeficientes de regressão, $\color{darkgreen}{(\beta_2 + \beta_5)}$ e $\color{darkgreen}{(\beta_3 + \beta_6)}$, têm interpretações semelhantes para o grupo de tratamento.

\framebreak

- Nos modelos de tendência polinomial, existe uma __hierarquia natural de efeitos__ que tem implicações para testar hipóteses sobre tendências polinomiais lineares, quadráticas e de ordem superior.
...
- Em vez disso, um __teste para a tendência quadrática (versus tendência linear)__ pode ser executada testando a hipótese nula de que $\beta_3 = 0$.
...
## Splines lineares {.allowframebreaks}
...
- Voltando ao estudo hipotético de dois grupos, comparando um __novo tratamento__ e um __controle__, se a resposta média mudar ao longo do tempo de maneira linear por partes, podemos ajustar o seguinte modelo de spline linear com o nó em $t^{*}$:
...
em que $(x)_{+}$, é conhecida como a __função de reta truncada__, e é definida como uma função que é \structure{igual a $x$ quando $x$ é positivo} e é __igual a zero caso contrário__. Assim,
...
- De maneira similar para indivíduos do grupo tratamento são dadas por
