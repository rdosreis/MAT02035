...
3. A "associação intra-indivídual" entre as respostas é uma função das médias e de parâmetros adicionais, digamos $\alpha$, que também precisam ser estimados.
...
- Observe que as restrições no _odds ratio_ são muito menos restritivas do que na correlação.
...
2. $\Var(Y_{ij}|X{ij}) = \phi_j$ (ou seja, variância heterogênea, mas nenhuma dependência da variância em relação à média).
...
2. $\Var(Y_{ij}|X{ij}) = \mu_{ij}(1 - \mu_{ij})$ (variância Bernoulli).
...
2. $\Var(Y_{ij}|X{ij}) = \phi\mu_{ij}$ (ou seja, variância extra-Poisson, ou "sobredispersão" quando $\phi > 1$).
...
- Abordagem alternativa para estimação: \structure{Equações de Estimação Generalizadas (GEE)}.
...
- A abordagem GEE é computacionalmente direta e foi implementada _softwares_ estatístico amplamente disponíveis.
...
- $D_i = \partial\mu_i/\partial\beta$ é a matriz "derivativa" (de $\mu_i$ em relação aos
componentes de $\beta$).
...
- Por exemplo, correlações não-estruturadas, $\alpha_{jk} =\Corr(Y_{ij}, Y_{ik})$, pode ser estiamda por
